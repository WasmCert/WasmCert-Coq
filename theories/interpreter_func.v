...
with run_one_step (fuel : fuel) (d : depth) (cfg : config_one_tuple_without_e) (e : administrative_instruction) : res_tuple :=
  ...
  | fuel.+1 =>
    ...
    | Basic (Binop t op) =>
      if ves is v2 :: v1 :: ves' then
        expect (app_binop op v1 v2)
               ...
               (hs, s, f, RS_normal ((vs_to_es ves') ++ [::Trap]))
      else (hs, s, f, crash_error)
    (* testops *)
    | Basic (Testop T_i32 testop) =>
      if ves is (ConstInt32 c) :: ves' then
        (hs, s, f, RS_normal (vs_to_es ((ConstInt32 (wasm_bool (@app_testop_i i32t testop c))) :: ves')))
      else (hs, s, f, crash_error)
    | Basic (Testop T_i64 testop) =>
      if ves is (ConstInt64 c) :: ves' then
        (hs, s, f, RS_normal (vs_to_es ((ConstInt32 (wasm_bool (@app_testop_i i64t testop c))) :: ves')))
      else (hs, s, f, crash_error)
    | Basic (Testop _ _) => (hs, s, f, crash_error)
    (* relops *)
    | Basic (Relop t op) =>
      if ves is v2 :: v1 :: ves' then
        (hs, s, f, RS_normal (vs_to_es (ConstInt32 (wasm_bool (app_relop op v1 v2)) :: ves')))
      else (hs, s, f, crash_error)
    (* convert & reinterpret *)
    ...
    (**)
    | Basic Unreachable => (hs, s, f, RS_normal ((vs_to_es ves) ++ [::Trap]))
    | Basic Nop => (hs, s, f, RS_normal (vs_to_es ves))
    | Basic Drop =>
      if ves is v :: ves' then
    ...
    | Basic (If tf es1 es2) =>
      if ves is ConstInt32 c :: ves' then
        if c == Wasm_int.int_zero i32m
        then (hs, s, f, RS_normal (vs_to_es ves' ++ [::Basic (Block tf es2)]))
        else (hs, s, f, RS_normal (vs_to_es ves' ++ [::Basic (Block tf es1)]))
      else (hs, s, f, crash_error)
    | Basic (Br j) => (hs, s, f, RS_break j ves)
    | Basic (Br_if j) =>
      if ves is ConstInt32 c :: ves' then
        if c == Wasm_int.int_zero i32m
        then (hs, s, f, RS_normal (vs_to_es ves'))
        else (hs, s, f, RS_normal (vs_to_es ves' ++ [::Basic (Br j)]))
      else (hs, s, f, crash_error)
    | Basic (Br_table js j) =>
      if ves is ConstInt32 c :: ves' then
        ...
        then
          expect (List.nth_error js k) (fun js_at_k =>
              (hs, s, f, RS_normal (vs_to_es ves' ++ [::Basic (Br js_at_k)])))
            (hs, s, f, crash_error)
        else (hs, s, f, RS_normal (vs_to_es ves' ++ [::Basic (Br j)]))
      else (hs, s, f, crash_error)
    | Basic (Call j) =>
      if sfunc s f.(f_inst) j is Some sfunc_i_j then
        (hs, s, f, RS_normal (vs_to_es ves ++ [::Invoke sfunc_i_j]))
      else (hs, s, f, crash_error)
    | Basic (Call_indirect j) =>
      if ves is ConstInt32 c :: ves' then
        ...
        | Some cl =>
          ...
          then (hs, s, f, RS_normal (vs_to_es ves' ++ [::Invoke cl]))
          else (hs, s, f, RS_normal (vs_to_es ves' ++ [::Trap]))
        | None => (hs, s, f, RS_normal (vs_to_es ves' ++ [::Trap]))
        end
      else (hs, s, f, crash_error)
    | Basic Return => (hs, s, f, RS_return ves)
    | Basic (Get_local j) =>
    ...
    | Basic (Tee_local j) =>
      if ves is v :: ves' then
        (hs, s, f, RS_normal (vs_to_es (v :: ves) ++ [::Basic (Set_local j)]))
      else (hs, s, f, crash_error)
    | Basic (Get_global j) =>
      ...
      expect
        ...
        (fun j =>
           if List.nth_error s.(s_mems) j is Some s_mem_s_j then
             (hs, s, f, RS_normal (vs_to_es (ConstInt32 (Wasm_int.int_of_Z i32m (Z.of_nat (mem_size s_mem_s_j))) :: ves)))
           else (hs, s, f, crash_error))
        (hs, s, f, crash_error)
    | Basic Grow_memory =>
      if ves is ConstInt32 c :: ves' then
    ...
    | Basic (EConst _) => (hs, s, f, crash_error)
    | Invoke cl =>
      match cl with
      | Func_native i (Tf t1s t2s) ts es =>
        ...
        then
          ...
          (hs, s, f, RS_normal (vs_to_es ves''
                  ++ [::Local m (Build_frame (rev ves' ++ zs) i) [::Basic (Block (Tf [::] t2s) es)]]))
        else (hs, s, f, crash_error)
      | Func_host (Tf t1s t2s) cl' =>
        let: n := length t1s in
        let: m := length t2s in
        if length ves >= n
        then
        ...
        | (hs', None) => (hs', s, f, RS_normal (vs_to_es ves ++ [::Invoke cl]))
          end
        else (hs, s, f, crash_error)
      end
    | Label ln les es =>
      if es_is_trap es
      then (hs, s, f, RS_normal (vs_to_es ves ++ [::Trap]))
      else
        if const_list es
        then (hs, s, f, RS_normal (vs_to_es ves ++ es))
          ...
          | RS_normal es' =>
            (hs', s', f', RS_normal (vs_to_es ves ++ [::Label ln les es']))
    ...
    | Local ln lf es =>
      ...
      then (hs, s, f, RS_normal (vs_to_es ves ++ [::Trap]))
        ...
        then
          ...
          | RS_normal es' =>
            (hs', s', f, RS_normal (vs_to_es ves ++ [::Local ln f' es']))
          | RS_crash error => (hs', s', f, RS_crash error)
          | RS_break _ _ => (hs', s', f, crash_error)
          end
    | Trap => (hs, s, f, crash_error)
    end
  end.
