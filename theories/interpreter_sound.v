...
Ltac explode_and_simplify :=
  ...
  | |- ?T = _ -> _ =>
    ...
    | context C [List.firstn ?n ?l] => rewrite (firstn_is_take_n)
...
Proof.
  ...
  (* Improved version, although still a bit clumsy *)
  (* Although used twice, this cannot be refactored as a stand-alone tactic since
     destruct of generic type is not allowed *) 
    lazymatch goal with
    | |- ?T -> _ =>
         lazymatch T with
    ...
    destruct n1 => //; first by destruct (n0 <= length l2).
    move => H. inversion H.
      (* Coq can only realise the huge expression is foldable after subst. *)
      by subst.
